...
const Usuarios = () => {
  ...
  const usuariosFiltrados = usuarios.filter((u) => {
    const nombreCompleto = `${u.nombre_usuario ?? ''} ${u.apellido_paterno ?? ''} ${u.apellido_materno ?? ''}`.toLowerCase();
    const correo = (u.correo ?? u.email ?? '').toLowerCase();
    const coincideBusqueda = busqueda === '' || nombreCompleto.includes(busqueda.toLowerCase()) || correo.includes(busqueda.toLowerCase());
  ...
  return (
    <div className="space-y-4">
      ...
      <div className={`rounded-lg p-4 border transition ${isDark
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          <div className="relative">
            ...
            <input
              ...
              className={`w-full pl-10 pr-4 py-2 border rounded-lg text-sm focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 transition ${isDark
            ...
            <tbody className={`divide-y ${isDark ? 'divide-slate-700' : 'divide-slate-200'}`}>
              {usuariosFiltrados.map((usuario) => (
                <tr key={usuario.id_usuario} className={`transition ${isDark ? 'hover:bg-slate-700' : 'hover:bg-slate-50'}`}>
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2">
                      <button className={`p-2 rounded-lg transition ${isDark
